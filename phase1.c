...
// insertRL()-- just flipped two operators when they compare priority
...
int fork1(char *name, int (*f)(char *), char *arg, int stacksize, int priority)
{
	...
	* the initial value of the process's program counter (PC) */
	...
	// If not sentinel process call dispatcher
...
void startup()
{
	...
	if (DEBUG && debugflag) console("startup(): calling fork1() for sentinel\n");
	...
	if (DEBUG && debugflag) console("startup(): calling fork1() for start1\n");
...
{
	check_mode();
	if (DEBUG && debugflag) console("in finish...\n");
}
/* finish */
...
// check if process has any time left for execution
...
static void check_deadlock()
{
   ...
   /* ------------------------------------------------------------------------
      ...
      Returns - nothing
      ----------------------------------------------------------------------- */
//Should be redone to follow ~format in output file
